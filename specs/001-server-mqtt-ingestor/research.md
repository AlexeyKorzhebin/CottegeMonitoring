...
Principle IV требует real-time доступ к актуальному срезу. При 10 домах × 150 объектов = 1500 записей state. API должен отвечать за <200ms.
...
## R-004: Docker и systemd деплой на Ubuntu

### Решение

Поддерживать **оба варианта** деплоя:
...
`docker-compose.yml` включает: app, postgres (+timescaledb), redis, mosquitto (dev only).

`docker-compose.prod.yml` — override для production (без mosquitto, внешний broker).

#### Systemd (альтернативный)

```ini
[Unit]
Description=CottageMonitoring MQTT Ingestor
After=network.target postgresql.service redis.service
Wants=postgresql.service redis.service

[Service]
Type=simple
User=cottage-monitoring
Group=cottage-monitoring
EnvironmentFile=/etc/cottage-monitoring/cottage-monitoring.env
WorkingDirectory=/opt/cottage-monitoring
ExecStart=/opt/cottage-monitoring/venv/bin/uvicorn cottage_monitoring.main:app --host 127.0.0.1 --port 8321
Restart=always
RestartSec=5
...
- Docker: изоляция, воспроизводимость, простота обновления.
- Systemd: для серверов без Docker или когда нужен прямой доступ к host-ресурсам.
...
## R-006: Nginx reverse proxy

### Решение

Сервис на порту **8321** (нестандартный, не конфликтует с типичными: 80, 443, 3000, 5432, 6379, 8080, 8443, 9090).

#### Конфигурация nginx

```nginx
upstream cottage_monitoring {
    server 127.0.0.1:8321;
}

server {
    listen 80;
    server_name monitoring.example.com;  # или IP

    location /api/ {
        proxy_pass http://cottage_monitoring;
...
- Достаточно высокий, чтобы не конфликтовать с другими проектами.
