...
- [x] T012 [P] Implement aiomqtt client wrapper with TLS support, login/password auth, configurable client_id (MQTT_CLIENT_ID), auto-reconnect with exponential backoff (1s..30s), wildcard subscription `{MQTT_TOPIC_PREFIX}cm/+/+/v1/#` in `server/src/cottage_monitoring/mqtt/client.py`
...
- [x] T044 [US3] Implement rpc_service: generate request_id, publish rpc/req to `{MQTT_TOPIC_PREFIX}cm/{house_id}/{device_id}/v1/rpc/req/{client_id}`, track pending requests, handle rpc/resp (including chunked), timeout for RPC requests in `server/src/cottage_monitoring/services/rpc_service.py`
