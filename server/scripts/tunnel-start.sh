...
TUNNEL_CMD="ssh -f -o ExitOnForwardFailure=yes -N \
  ...
  -L 8321:localhost:8321 \
  -L 8322:localhost:8322 \
  elion"

if pgrep -f "ssh.*5432:localhost:5432.*6379:localhost:6379.*elion" >/dev/null; then
  echo "SSH tunnel to elion already running (ports 5432, 6379, 1883, 8321, 8322)"
  exit 0
fi

echo "Starting SSH tunnel to elion..."
$TUNNEL_CMD
echo "Tunnel started. Ports: 5432 (PG), 6379 (Redis), 1883 (MQTT), 8321 (REST prod), 8322 (REST dev)"
